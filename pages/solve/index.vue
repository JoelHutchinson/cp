<template>
  <!-- No puzzle sets -->
  <div v-if="puzzleSets && puzzleSets.length === 0" class="p-6 sm:p-0 flex items-center justify-center size-full">
    <UiTypography>
      No puzzles sets have been created yet. Please
      <UButton to="/puzzle-set" variant="link" class="p-0 m-0">create a puzzle set</UButton>
      to start solving.
    </UiTypography>
  </div>

  <!-- prettier-ignore -->
  <div v-else class="size-full grid
    ...
    <!-- Puzzle Details -->
    <div class="row-start-1 row-end-2 sm:row-start-2 sm:row-end-3 sm:col-start-2 sm:col-end-3 h-full">
      <ChessPuzzleDetails v-if="puzzleBoardState" :puzzle-board-state="puzzleBoardState" @next-move="handleNext"
        @prev-move="handlePrev" @skip="solvePuzzle" class="h-full" />

      <USkeleton v-else class="h-full" />
    </div>

    <!-- Puzzle Set Details -->
    <div class="row-start-3 row-end-4 sm:row-start-1 sm:row-end-2 sm:col-start-2 sm:col-end-3 h-full">
      <ChessPuzzleSetDetails v-if="selectedPuzzleSetSlug && puzzleSets && puzzleSetProgress"
        v-model:selected-puzzle-set-slug="selectedPuzzleSetSlug" :puzzle-sets="puzzleSets"
        :puzzle-set-progress="puzzleSetProgress" class="size-full" />
